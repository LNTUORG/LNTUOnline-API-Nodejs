...
var getDOMTypeNormal = function(callback) {
  domAgent.main(info.reqUrl, info.userId, info.password, function(html) {
    callback(null, html);
  })
};
...
var analyseHTML = function(html, callback) {
  ...
  if (html == agentConfig.ERROR_INFO.NET_ERROR) {
    laStudent = agentConfig.ERROR_INFO.NET_ERROR;
    callback(null, laStudent);
  } else if (html == agentConfig.ERROR_INFO.ACCOUNT_ERROR) {
    laStudent = agentConfig.ERROR_INFO.ACCOUNT_ERROR;
    callback(null, laStudent);
  } else {
